...
angle_cap = 100  # 카메라 화각 (위아래로 촬영 가능한 총 각도)
...
while(True):
    ...
    if(end_mode == 1 or end_mode == 2):
        ...
        elif USRID.rfind('normal') != -1 and end_mode==2:
            move_path = origin_path[:-6] +"easy"
            replace_dir ="mv "+origin_path+" "+move_path
            os.system(replace_dir)
        elif USRID.rfind('easy') != -1 and end_mode==1:
