...
frameWidth = 100
frameHeight = 100
imamge = cv.imread('test.jpg')
cv.imshow('test', imamge)
cap = cv.VideoCapture(0)
cap.set(cv.CAP_PROP_FRAME_WIDTH, frameWidth) # Width
cap.set(cv.CAP_PROP_FRAME_HEIGHT, frameHeight) # Heigh
...
while True:
    ...
    if step == 1:
        cv.circle(img, (start_x, start_y), 10, (0, 255, 0), -1)

    elif step == 2:
        # 왼쪽 버튼을 누른 채 움직이고 있는 영역을 초록색 사각형으로 보여준다
        cv.rectangle(img, (start_x, start_y),
